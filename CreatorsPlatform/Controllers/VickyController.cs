...
namespace CreatorsPlatform.Controllers
{
    public class VickyController : Controller
    {
        ...
        public IActionResult Search(string searchKey, int page = 1, int pageSize = 3)
        {
            var vickyWorkContent = new VickyWorkContent(_context);
            var workList = vickyWorkContent.GetSearchContents(searchKey, page, pageSize);
            var totalItems = workList.Count(); // 获取总条目数
